...
def dataframe_time(formatted_beer):
    ...
    df.columns = ['Name', 'Brewer', 'Style/ABV',
                  'Ratings', 'More Ratings', 'Review']
    return df
...
def fix_ratings(df):
    # Now we have the Style and ABV fields. Let's get the ratings section
    # formatted. We are only concerned with the average rating, so we will
    # extract that.
    ratings = []
...
def smell_feel(df):
    ...
    # OUTPUT: same df with 'More Ratings' columns replaced with a smell and a
    # feel column
